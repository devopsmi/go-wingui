...
1.windows7
2.go1.3.3 (32位)
3.MinGW（按着官方说明进行安装），安装到C:\MinGW

### clone代码  
1.在D:盘新建gowork目录，然后cmd进入gowork，执行：git clone 
2.

#  路径
为了在IDE中能够正确识别包路径，执行下面操作：
1.把路径D:\gowork\cef2go\src追加到系统的GOPATH里面（GOPATH支持多个路径，用冒号分隔即可）
2.把路径D:\gowork\cef2go\Release追加到系统的PATH里面
